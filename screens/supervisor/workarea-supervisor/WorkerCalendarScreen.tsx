...
// ─── Types ────────────────────────────────────────────────────────────────────
type Props = NativeStackScreenProps<SupervisorStackParamList, "WorkerCalendar">;
...
// ─── Main Screen ──────────────────────────────────────────────────────────────
export default function WorkerCalendarScreen({ navigation, route }: Props) {
  // workerId và workerName lấy từ route params — không cần fetch từ auth
  ...
  const fetchMonthTasks = useCallback(async () => {
    ...
    try {
      ...
      console.warn("[WorkerCalendar] fetch failed", e);
    } finally {
      setLoading(false);
    }
  }, [workerId, year, month]);
            ...
            <TouchableOpacity activeOpacity={1}>
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  <SlotItem task={item} index={index} />
                )}
                ListEmptyComponent={
